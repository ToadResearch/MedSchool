...
#   [WRAPPER]  POST /validator/validate      (sync)  → OperationOutcome
#              POST /validator/requests      (async) → poll until OperationOutcome
#
# IMPORTANT BEHAVIOR:
...
[[ ${1:-} == "--url" && -n ${2:-} ]] && { BASE_URL="$2"; ORIGIN=$(printf '%s' "$BASE_URL" | sed -E 's#/fhir/?$##'); VALIDATOR_BASE="${ORIGIN}/validator"; shift 2; }
...
validator_validate_sync() { # label jsonVar expectError(0|1)
  ...
  if [[ "$code" != "200" ]]; then
    echo -e " ${YLW}SKIP${RST} (HTTP $code on /validate; will try async /requests)" >&2
    rm -f "$body"
    return 99
  fi
  ...
  if jq -e '.resourceType=="OperationOutcome"' <"$body" >/dev/null 2>&1; then
    is_oo=1
    jq -e '.issue[]? | select(.severity=="error" or .severity=="fatal")' <"$body" >/dev/null || true
    [[ $? -eq 0 ]] && has_error=1
  elif jq -e '.outcome.resourceType=="OperationOutcome"' <"$body" >/dev/null 2>&1; then
    is_oo=1
    jq -e '.outcome.issue[]? | select(.severity=="error" or .severity=="fatal")' <"$body" >/dev/null || true
    [[ $? -eq 0 ]] && has_error=1
  fi
  ...
  if [[ "$expect_err" -eq 1 && "$has_error" -eq 1 ]]; then
    echo -e " ${GRN}✔${RST} (sync OO has errors as expected)" >&2
  elif [[ "$expect_err" -eq 0 && "$has_error" -eq 0 ]]; then
    echo -e " ${GRN}✔${RST} (sync OO has no errors as expected)" >&2
  else
...
# Async flow: POST /validator/requests → poll Location or id → OO
validator_validate_async() { # label jsonVar expectError(0|1)
  ...
  local code; code=$(curl -sS -o "$body" -D "$hdrs" -w "%{http_code}" \
        ...
        -H "Authorization: Bearer $FHIR_BEARER_TOKEN" \
        -H "Accept: application/json, application/fhir+json" \
  ...
  while (( tries < max_tries )); do
    ...
    if jq -e '.resourceType=="OperationOutcome"' <"$poll_body" >/dev/null 2>&1; then
      got_oo=1
      jq -e '.issue[]? | select(.severity=="error" or .severity=="fatal")' <"$poll_body" >/dev/null || true
      [[ $? -eq 0 ]] && has_error=1
      rm -f "$poll_body"; break
    elif jq -e '.outcome.resourceType=="OperationOutcome"' <"$poll_body" >/dev/null 2>&1; then
      got_oo=1
      jq -e '.outcome.issue[]? | select(.severity=="error" or .severity=="fatal")' <"$poll_body" >/dev/null || true
      [[ $? -eq 0 ]] && has_error=1
      rm -f "$poll_body"; break
    fi
  ...
  if [[ "$expect_err" -eq 1 && "$has_error" -eq 1 ]]; then
    echo -e " ${GRN}✔${RST} (async OO has errors as expected)" >&2
  elif [[ "$expect_err" -eq 0 && "$has_error" -eq 0 ]]; then
    echo -e " ${GRN}✔${RST} (async OO has no errors as expected)" >&2
  else
...
validator_validate() { # label jsonVar expectError(0|1)
  local label="$1" var="$2" expect_err="$3"
  validator_validate_sync "$label (sync)" "$var" "$expect_err" && return 0
  [[ $? -eq 99 ]] && validator_validate_async "$label (async)" "$var" "$expect_err" && return 0
  return 1
}
...
# Validate good + bad resources via wrapper (prefers /validate; falls back to /requests)
